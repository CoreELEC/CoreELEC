...
static int _hybris_should_trace = 1;
...
{
    hybris_logging_target = fopen("/storage/backup/libhybris.log", "a");
    if (hybris_logging_target == NULL)
        hybris_logging_target = stderr;
/*
    const char *env = NULL;
    setenv("HYBRIS_LOGGING_LEVEL", "debug", 1);
    setenv("HYBRIS_LOGGING_TARGET", "/storage/backup/libhybris.log", 1);
    setenv("HYBRIS_TRACE", "1", 1);

    env = getenv("HYBRIS_LOGGING_LEVEL");
