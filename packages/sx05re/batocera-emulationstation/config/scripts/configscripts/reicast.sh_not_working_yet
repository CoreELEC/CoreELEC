...
function onstart_reicast_joystick() {
    ...
    case "$DEVICE_NAME" in
        "Xbox 360 Controller (xpad driver)")
            file="$configdir/reicast/mappings/controller_xpad.cfg"
            ;;
        "Xbox 360 Controller (xboxdrv userspace driver)")
            file="$configdir/reicast/mappings/controller_xboxdrv.cfg"
            ;;
        *)
            file="$configdir/reicast/mappings/${DEVICE_NAME}.cfg"
            ;;
    esac

    # create mapping dir if necessary.
    mkdir -p "$configdir/reicast/mappings"
